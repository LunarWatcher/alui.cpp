...
* A C++23 compiler
* allegro, with `allegro_font`, `allegro_color`, `allegro_primitives`, `allegro_image`.
    ...
    * Note that internally, these targets are linked as-is. It's up to the invoking CMakeLists to ensure these targets exist, or for files by those names to be discoverable by the linker. This may change if a better way to link is found.
