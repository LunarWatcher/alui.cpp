...
struct AbstractStyle {
    ...
    virtual void render(Component*, float scrollX, float scrollY) = 0;
};
...
struct StyleStates {
    ...
    void render(Component* component, float scrollX, float scrollY) {
        ...
        if ((*state).has_value()) {
            (**state)->render(component, scrollX, scrollY);
        }
    }
};
